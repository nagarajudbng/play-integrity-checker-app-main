...
//            new getTokenResponse().execute(integrityToken);
            ...
            try {
                String token = integrityTokenResponse1.token();
                ...
                PlayIntegrity.Builder playIntegrity = new PlayIntegrity.Builder(HTTP_TRANSPORT, JSON_FACTORY, requestInitializer).setApplicationName("testintegritycloud")
                ...
                Thread thread = new Thread(new Runnable() {
                    ...
                    public void run() {
                        try  {
                            DecodeIntegrityTokenResponse response = play.v1().decodeIntegrityToken("gr.nikolasspyr.integritycheck", requestObj).execute();
...
//                            showErrorDialog("Error getting token from Google", getErrorText(e));
        ...
        integrityTokenResponse.addOnCompleteListener( it->{
            if (it.isSuccessful()) {
                Log.d("YAYAYA", "success");
                Log.d("YAYAYA", it.toString());
                Log.d("YAYAYA", it.getResult().toString());
                Log.d("YAYAYA", it.getResult().token());
            } else {
                Log.d("YAYAYA", "exception");
                Log.d("YAYAYA", it.getException().toString());
            }
        }
        );
